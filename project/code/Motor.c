...
{
	gpio_init(MOTOR1_DIR, GPO, GPIO_HIGH, GPO_PUSH_PULL);                            // GPIO 初始化为输出 默认上拉输出�?
    pwm_init(MOTOR1_PWM, 17000, 0);                                                  // PWM 通道初始化频�?17KHz 占空比初始为 0
    
    gpio_init(MOTOR2_DIR, GPO, GPIO_HIGH, GPO_PUSH_PULL);                            // GPIO 初始化为输出 默认上拉输出�?
    pwm_init(MOTOR2_PWM, 17000, 0);                                                  // PWM 通道初始化频�?17KHz 占空比初始为 0

    gpio_init(MOTOR3_DIR, GPO, GPIO_HIGH, GPO_PUSH_PULL);                            // GPIO 初始化为输出 默认上拉输出�?
    pwm_init(MOTOR3_PWM, 17000, 0);                                                  // PWM 通道初始化频�?17KHz 占空比初始为 0

    gpio_init(MOTOR4_DIR, GPO, GPIO_HIGH, GPO_PUSH_PULL);                            // GPIO 初始化为输出 默认上拉输出�?
    pwm_init(MOTOR4_PWM, 17000, 0);                                                  // PWM 通道初始化频�?17KHz 占空比初始为 0
}

void encoder_init(void)
{
	encoder_quad_init(ENCODER_1, ENCODER_1_A, ENCODER_1_B);                     // 初始化编码器模块与引�?正交解码编码器模�?
    encoder_quad_init(ENCODER_2, ENCODER_2_A, ENCODER_2_B);                     // 初始化编码器模块与引�?正交解码编码器模�?
    encoder_quad_init(ENCODER_3, ENCODER_3_A, ENCODER_3_B);                     // 初始化编码器模块与引�?正交解码编码器模�?
    encoder_quad_init(ENCODER_4, ENCODER_4_A, ENCODER_4_B);                     // 初始化编码器模块与引�?正交解码编码器模�?
}
...
int16 down_L_all=0;
int16 up_R_all=0;
int16 down_R_all=0;//编码器积分变�?
int32 encoder_all=0;
int16 encoders_average;//积分平均�?

int16 encoder_data_quaddec1 = 0;//编码器的�?
...
void encoder_get(void)
{
	...
	encoder_data_quaddec1 = -encoder_raw_quaddec4;                  // 获取编码器计�?左上
	encoder_data_quaddec2 = -encoder_raw_quaddec3;                  // 获取编码器计�?右上
	encoder_data_quaddec3 = -encoder_raw_quaddec1;                  // 获取编码器计�?左下
	encoder_data_quaddec4 = -encoder_raw_quaddec2;                  // 获取编码器计�?右下
#else
	encoder_data_quaddec1 = -encoder_raw_quaddec1;                  // 获取编码器计�?左上
	encoder_data_quaddec2 = -encoder_raw_quaddec2;                  // 获取编码器计�?右上
	encoder_data_quaddec3 = -encoder_raw_quaddec3;                  // 获取编码器计�?左下
	encoder_data_quaddec4 = -encoder_raw_quaddec4;                  // 获取编码器计�?右下
#endif
	...
	encoder_L_up[4]=encoder_L_up[3];//左上编码�?
	...
	encoder_R_up[4]=encoder_R_up[3];//右上编码�?
	...
	encoder_L_down[4]=encoder_L_down[3];//左下编码�?
	...
	encoder_R_down[4]=encoder_R_down[3];//右下编码�?
	...
	encoder_clear_count(ENCODER_1);                                       // 清空编码器计�?
	encoder_clear_count(ENCODER_2);                                       // 清空编码器计�?
	encoder_clear_count(ENCODER_3);                                       // 清空编码器计�?
...
void motor_pwm(int up_left_speed,int up_right_speed,int down_left_speed,int down_right_speed)
{
    ...
    {
		gpio_set_level(MOTOR1_DIR, GPIO_LOW);                     // DIR输出高电�?
        pwm_set_duty(MOTOR1_PWM, up_left_speed);                   // 计算占空�?
     }
     else if (up_left_speed < 0)                                                                  // 反转
     {
		gpio_set_level(MOTOR1_DIR, GPIO_HIGH);                    // DIR输出低电�?
        pwm_set_duty(MOTOR1_PWM, -up_left_speed);                // 计算占空�?
	 ...
	 if (up_right_speed > 0)
	 {
		 gpio_set_level(MOTOR2_DIR, GPIO_HIGH);                       // DIR输出高电�?
         pwm_set_duty(MOTOR2_PWM, up_right_speed);                   // 计算占空�?
	 }
	 else if (up_right_speed < 0)
	 {
		 gpio_set_level(MOTOR2_DIR, GPIO_LOW);                     // DIR输出低电�?
         pwm_set_duty(MOTOR2_PWM, -up_right_speed);                // 计算占空�?
	 }
	 else if (up_right_speed == 0)
	 {
	 ...
	 {
		 gpio_set_level(MOTOR3_DIR, GPIO_LOW);                       // DIR输出高电�?
         pwm_set_duty(MOTOR3_PWM, down_left_speed);                   // 计算占空�?
	 }
	 else if (down_left_speed < 0)
	 {
		 gpio_set_level(MOTOR3_DIR, GPIO_HIGH);                      // DIR输出低电�?
         pwm_set_duty(MOTOR3_PWM, -down_left_speed);                // 计算占空�?
	 }
	 else if (down_left_speed == 0)
	 {
	 ...
	 {
		 gpio_set_level(MOTOR4_DIR, GPIO_HIGH);                       // DIR输出高电�?
         pwm_set_duty(MOTOR4_PWM, down_right_speed);                  // 计算占空�?
	 }
	 else if (down_right_speed < 0)
	 {
		 gpio_set_level(MOTOR4_DIR, GPIO_LOW);                       // DIR输出低电�?
         pwm_set_duty(MOTOR4_PWM, -down_right_speed);                // 计算占空�?
	 }
	 else if (down_right_speed == 0)
	 {
