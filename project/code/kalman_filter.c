...
//================================================================================================================================
//原作者是利用malloc进行内存分配，这里为了稳妥起见，使用全局变量，这样方便到各个单片机系统上。 仅仅智能车使用
#define Kalman_xhatSize 6
#define Kalman_uSize 0
#define Kalman_zSize 3

// measurement flags
uint8_t MeasurementMap_Temp[Kalman_zSize]={0};
float MeasurementDegree_Temp[Kalman_zSize]={0};
float MatR_DiagonalElements_Temp[Kalman_zSize]={0};
float StateMinVariance_Temp[Kalman_xhatSize]={0};
uint8_t temp_Temp[Kalman_zSize]={0};

// filter data
float FilteredValue_Temp[Kalman_xhatSize]={0};
float MeasuredVector_Temp[Kalman_zSize]={0};

// xhat x(k|k)
float xhat_data_Temp[Kalman_xhatSize]={0};

// xhatminus x(k|k-1)
float xhatminus_data_Temp[Kalman_xhatSize]={0};

// measurement vector z
float z_data_Temp[Kalman_zSize]={0};

// covariance matrix P(k|k)
float P_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};

// create covariance matrix P(k|k-1)
float Pminus_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};

// state transition matrix F FT
float FT_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};
float F_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};

// measurement matrix H
float HT_data_Temp[Kalman_xhatSize*Kalman_zSize]={0};
float H_data_Temp[Kalman_xhatSize*Kalman_zSize]={0};

// process noise covariance matrix Q
float Q_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};

// measurement noise covariance matrix R
float R_data_Temp[Kalman_zSize*Kalman_zSize]={0};

// kalman gain K
float K_data_Temp[Kalman_zSize*Kalman_xhatSize]={0};

float S_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};
float temp_matrix_data_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};
float temp_matrix_data1_Temp[Kalman_xhatSize*Kalman_xhatSize]={0};
float temp_vector_data_Temp[Kalman_xhatSize]={0};
float temp_vector_data1_Temp[Kalman_xhatSize]={0};
/**
...
void Kalman_Filter_Init(KalmanFilter_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
    ...
    kf->xhatSize = Kalman_xhatSize;
    kf->uSize = Kalman_uSize;
    kf->zSize = Kalman_zSize;
    ...
    kf->xhatminus_data=xhatminus_data_Temp;
    ...
    kf->Pminus_data=Pminus_data_Temp;
    ...
    kf->F_data=F_data_Temp;
    kf->FT_data=FT_data_Temp;
    ...
    kf->H_data=H_data_Temp;
    kf->HT_data=HT_data_Temp;
    ...
    kf->temp_matrix_data=temp_matrix_data_Temp;
...
/**
 * @brief 复位卡尔曼滤波器，把滤波器内部数据清零，保留对象本身，适合重新开始滤波
 *
 * @param kf类型定义
 * @param xhatSize 状态变量维度
 * @param uSize 控制变量维度
...
//把外部写入的 MeasuredVector 转成内部量测 z。如果 UseAutoAdjustment=1，会调用自动重构逻辑；否则直接拷贝
