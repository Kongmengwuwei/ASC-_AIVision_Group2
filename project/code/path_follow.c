...
#define PATH_FOLLOW_TARGET_X_FROM_Y_RUN_COMP_K 0.025f
...
#define PATH_ROTATE_SLOWDOWN_BAND_DEG 10.0f
#define PATH_ROTATE_SLOWDOWN_MIN_DEGPS 4.0f // 小于15度最大角速度
...
/* [CornerHandover] 单个拐点交接最多允许持续的时间，单�?s�?*/
 ...
 */
#define PATH_FOLLOW_MAX_ANGULAR_SPEED_LIMIT 90.0f
...
#if PATH_FOLLOW_COMPILE_CORNER_HANDOVER
    uint8 corner_handover_active;   // 1: 当前普通中间拐点已进入 handover �?
    size_t corner_handover_idx;     // 当前 handover 对应�?target idx
...
/**
 * @brief �?`pid_stay` 的参数同步到 `pid_stay_y`�?
 *
...
static uint8 path_follow_is_push_span_end_marker(uint8 marker_id)
{
...
static uint8 path_follow_current_segment_is_push(void)
{
    ...
    for (i = 0U; (i + 1U) < g_ctx.steps && (i + 1U) <= g_ctx.idx; i++)
    {
        if (path_follow_is_push_span_end_marker(g_ctx.path[i].id))
        {
            push_active = 0U;
        }
        ...
        if (path_follow_is_push_span_end_marker(g_ctx.path[i + 1U].id))
        {
            push_active = 0U;
        }
    }
                                   ...
                                   path_follow_speed_plan_t *speed_plan)
{
    ...
    if (path_follow_current_segment_is_push() &&
        ...
        speed_plan->safety_cap_cmps > 0.0f)
    {
        float push_min_cmps = fminf(PATH_FOLLOW_PUSH_MIN_CMPS,
                                    speed_plan->safety_cap_cmps);
 ...
 * @param vx_world_cmps 世界�?X 方向速度输出�?
 * @param vy_world_cmps 世界�?Y 方向速度输出�?
