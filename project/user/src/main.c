...
int main(void)
{
  ...
  interrupt_set_priority(LPUART4_IRQn, 8);   // New-board vision camera on UART4.
  /* New-board UART8 priority and RX interrupt state are owned by BlueSerial. */
#else
  interrupt_set_priority(LPUART8_IRQn, 8);   // Old-board UART8: Bluetooth or vision camera.
#endif
